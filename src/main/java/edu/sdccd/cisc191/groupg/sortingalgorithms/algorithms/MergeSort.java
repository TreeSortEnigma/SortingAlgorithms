...
*/
 ...
 import edu.sdccd.cisc191.groupg.sortingalgorithms.SortManager;
    ...
    void merge (int arr[], int left, int middle, int right, SortManager sortManager) {
    ...
    // Main function that sorts arr[l..r] using merge()
        ...
        void sort (int array[], int left, int right, SortManager sortManager) {
            if (left < right) {
                // Find the middle point
                int middle = (left + right) / 2;

                //Sort first and second halves
                sort(array, left, middle, sortManager);
                sort(array, middle + left, right, sortManager);

                //Merge the sorted halves
                merge(array, left, middle, right, sortManager);
            }
        }
